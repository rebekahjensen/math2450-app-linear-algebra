...
D = house_sales_data() #loads data set
...
X = np.column_stack([x[0]**2, x[1]**2, x[0]*x[1], x[0], x[1], np.ones(m)]) #EDIT I MADE FOR FEAUTURES X, from example!
...
coeff, train_error, test_error = cross_validate(X, y, folds = 5) #IMPORTANT EDIT I MADE, ensures the 5-fold cross validation :)
avg_test_error = np.mean(test_error) #I THINK THIS IS WHAT THE ASSIGNMENT WANTS ME TO DO
print("Average prediction error for 5-fold:", avg_test_error) #printing my understanding
print("This looks at how the combination of area and number of bedrooms affects the price of a house, while example 4.3.2 looks at a simpler linear model which only compares direct lienar relationships!")
